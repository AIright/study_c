...
class TeamTasks {
private:
    map<string, TasksInfo> tasks_database;
public:
    // Получить статистику по статусам задач конкретного разработчика
    const TasksInfo& GetPersonTasksInfo(const string& person) const {
        if (tasks_database.count(person)) {
            return tasks_database.at(person);
        }
        return TasksInfo{};
    };
    ...
    void AddNewTask(const string& person) {
        tasks_database[person][TaskStatus::NEW]++;
    };
    ...
    tuple<TasksInfo, TasksInfo> PerformPersonTasks(
        ...
        // Prepare buffer for tasks count
        TasksInfo person_tasks_count = tasks_database.at(person);
        TasksInfo updated, not_updated;
        int new_tasks = person_tasks_count[TaskStatus::NEW];
        int in_progress_tasks = person_tasks_count[TaskStatus::IN_PROGRESS];
        int testing_tasks = person_tasks_count[TaskStatus::TESTING];
        ...
        if (new_tasks != 0) {
            not_updated[TaskStatus::NEW] = new_tasks;
        }
        if (in_progress_tasks != 0) {
            not_updated[TaskStatus::IN_PROGRESS] = in_progress_tasks;
        }
        if (testing_tasks != 0) {
            not_updated[TaskStatus::TESTING] = testing_tasks;
        }

        // Update database (every incomplete task is a sum of updated and not updated
        tasks_database[person][TaskStatus::NEW] = not_updated[TaskStatus::NEW];
        tasks_database[person][TaskStatus::IN_PROGRESS] =
                updated[TaskStatus::IN_PROGRESS] + not_updated[TaskStatus::IN_PROGRESS];
        tasks_database[person][TaskStatus::TESTING] =
                updated[TaskStatus::TESTING] + not_updated[TaskStatus::TESTING];
        tasks_database[person][TaskStatus::DONE] += updated[TaskStatus::DONE];

        // Erase zeros
        for (auto i : tasks_database[person]) {
            if (i.second == 0) {
                tasks_database[person].erase(i.first);
            }
        }
...
Если task_count превышает текущее количество невыполненных задач разработчика, достаточно считать,
что task_count равен количеству невыполненных задач:
...
При возникновении такой необходимости рекомендуется сначала в отдельном временном
словаре собрать информацию об обновлениях, а затем применить их к основному словарю.
*/
//Пример кода
//
